       ...
       if (posti_cosigliati(prenotazione , fila , posto)==1){
        ...
        if (occupato==false ){
            for (int i = 0; i < (posto_finale-posto_iniziale)+1; ++i) {
                ...
                out << prenotazione[fila][posto_iniziale+i].nome << ";" << prenotazione[fila][posto_iniziale+i].cognome << ";" << +1 << ";" << posto_iniziale+i+1
                    << ";" << endl;
...
int main() {
    ...
    in.close();
    cout<<remove("prenotazioni_precedenti.txt");
    out.close();
    cout<<rename("prenotazioni.txt","prenotazioni_precedenti.txt");
    return 0;
}